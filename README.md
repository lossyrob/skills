...
- Dissent-preserving synthesis: returns a required-field packet (recommendation, confidence basis, decisive arguments with sources, minority report, open questions, reopen conditions, audit triggers, provenance manifest, artifact paths); the main session always surfaces minority report and reopen conditions, and a faithfulness check guards rapporteur fidelity
- Context containment: defaults to a single contained council-runner subagent that spawns members, runs the rounds in its own context, flushes the transcript turn-by-turn, and returns only a provenance-bearing synthesis; the main session reads synthesis by default and audits the transcript on request (driver-launched peers are a portability fallback)
